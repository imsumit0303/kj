...
app.controller("myCtrl", function($scope, $http, $location, $rootScope) {
   ...
   /*$scope.hsubmit=function(){
    ...
    if(emailReg.test($scope.emailNum) || mobileRegex.test($scope.emailNum)){
      ...
      $http.post("http://192.168.31.166:8080/user_register", data).
 ...
 $scope.hsubmit=function(){
  ...
  /*student ambassador verification*/
...
/*$scope.csend=function(){
  ...
  $http.post("http://192.168.31.166:8080/contactus", data).
    success(function(data, status, header, config){   
    console.log("success data "+ data)
    console.log("success "+data.status_code);
    if(data.status_code==200){
...
$scope.csend=function(){
  ...
  $http({
     ...
     //cphone.style.border="1px solid red";
  ...
  $scope.startOrStop = function(){
   ...
   /*$scope.start=5000;
   $scope.addCDSeconds=5;*/
    /* $scope.dynamic1='clLink';*/
    ...
    /* $scope.addCD=7; */
     ...
     $scope.$on('timer-stopped', function (event, args) {
      ...
      /*$scope.dynamic1.color='red';*/
      console.log("tstop called");
      console.log('$scope.dynamic1: %j', $scope);
      console.log("otp value: "+$scope.user.otp);
      $scope.dynamic1= {"color":"red"};
     ...
     $scope.send=function(){
      var ambass;
      console.log('$scope.user.check1', $scope.user.check1);
      if ($scope.user.check1) {
        ambass = 1;
      } else {
        ambass = 0;
      }
      ...
      var data1={
       ...
       ambassador: ambass ,
       suggesstion: $scope.user.message

     };

     $http.post("http://192.168.31.166:8080/web_otp_verify",data1).
     success(function(data,status,header,config){
      console.log("after success f1");
      if(data.success==1){
        console.log("after success if");
        $scope.otp_matched=true;
        $scope.message1="OTP matched";
      }
      else{

        $scope.otp_matched=true;
        $scope.message1="something wrong";
        console.log("after success else");
        console.log('$scope.otp matched: %j', $scope);
        /*$scope.$apply();*/
     ...
     error(function (data, status, headers, config) {
      ...
      alert("something wrong during the AJAX request");
    });
   };
    ...
    $scope.resend=function(){
      ...
      $scope.dynamic1={"color":"grey"};
      /*$rootScope.$digest();*/
      /* $scope.$broadcast('timer-clear');*/
      $scope.$broadcast('timer-reset');
      /* $scope.addCD=8;
      $rootScope.$broadcast('timer-reset',$scope.addCD);*/

      $scope.$broadcast('timer-add-cd-seconds', 1);
      console.log("addCD" +$scope.addCD);
      /*$scope.*/
      /* $scope.addCD=8;*/
      $scope.$broadcast('timer-start');

      console.log("reesend mobile"+ $scope.user.mobile);
      var data={
        contact: $scope.user.mobile 
      };

      $http.post("http://192.168.31.166:8080/web_register_resendotp", data).
      success(function(data, status, header, config){   
        console.log("success data "+ data)
        console.log("success "+data.success);
        if(data.success==1){
         /* $location.path('/otp');*/
         $rootScope.hidefrm= true;
         $scope.ShowOtpForm;
         $rootScope.ShowOtpForm = true;
         console.log("show next form "+$scope.ShowOtpForm);  
    ...
    $scope.submit=function(){
      ...
      $scope.dynamic1={"color":"grey"};
      console.log("$scope.dynamic1: "+$scope.dynamic1);
      var data={
      ...
      $http.post("http://192.168.31.166:8080/web_student_register", data).
      success(function(data, status, header, config){   
        console.log("success data "+ data)
        console.log("success "+data.success);
        if(data.success==1){
...
app.controller('hideHeaderCtrl', function ( $window, $rootScope, $scope, $location, $timeout, $cacheFactory, $anchorScroll,) {
  ...
  //     console.log( Math.abs(checkScrollSpeed()) );
  //   };
  //   document.getElementById("headerWrap").style.transition = "top 0.5s";
